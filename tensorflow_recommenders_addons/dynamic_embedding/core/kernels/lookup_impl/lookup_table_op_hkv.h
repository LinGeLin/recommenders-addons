...
class TableWrapper {
 ...
 public:
  TableWrapper(TableWrapperInitOptions& init_options, size_t dim,
    ...
    // auto ev = static_cast<HkvEvictStrategy>(strategy);
    if (HkvEvictStrategy::kLfu == strategy) {
      table_ =
          new nv::merlin::HashTable<K, V, uint64_t, HkvEvictStrategy::kLfu>();
    } else if (HkvEvictStrategy::kEpochLru == strategy) {
      table_ = new nv::merlin::HashTable<K, V, uint64_t,
                                         HkvEvictStrategy::kEpochLru>();
    } else if (HkvEvictStrategy::kEpochLfu == strategy) {
      table_ = new nv::merlin::HashTable<K, V, uint64_t,
                                         HkvEvictStrategy::kEpochLfu>();
    } else if (HkvEvictStrategy::kCustomized == strategy) {
      table_ = new nv::merlin::HashTable<K, V, uint64_t,
