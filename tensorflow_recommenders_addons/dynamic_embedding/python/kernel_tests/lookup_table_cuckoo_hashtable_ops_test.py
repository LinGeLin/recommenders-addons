...
class CuckooHashtableTest(test.TestCase):
        ...
        for dev_str, use_gpu, init_size, expect_size in test_list:
            with self.session(use_gpu=use_gpu, config=default_config):
                with self.captureWritesToStream(sys.stderr) as printed:
                    if use_gpu:
                        os.environ["LOOKUP_TABLE_NAME"] = "CUCKOO_HASHTABLE_GPU"
                    else:
                        os.environ["LOOKUP_TABLE"] = "CUCKOO_HASHTABLE"
            ...
            with self.session(use_gpu=use_gpu, config=default_config):
                with self.captureWritesToStream(sys.stderr) as printed:
                    ...
                    values = constant_op.constant(
                        [[1, 1, 1] for _ in range(168)], dtypes.int32
                    )
    ...
    def test_cuckoo_hashtable_save_file_system(self):
        ...
        test_devices = ["/CPU:0"]
        os.environ["LOOKUP_TABLE_NAME"] = "CUCKOO_HASHTABLE"
        if test_util.is_gpu_available():
            os.environ["LOOKUP_TABLE"] = "CUCKOO_HASHTABLE_GPU"
            test_devices = ["/GPU:0"]
        dim = 8
        for idx, device in enumerate(test_devices):
            ...
            with self.session():
                ...
                self.evaluate(
                    var1.tables[0].save_to_file_system(
                        dirpath, file_name="fsv_" + str(idx), buffer_size=4096
                    )
                )
                self.evaluate(
                    var2.tables[0].load_from_file_system(
                        dirpath, file_name="fsv_" + str(idx), buffer_size=4096
                    )
                )
    ...
    def test_cuckoo_hashtable_save_local_file_system(self):
        ...
        for idx, device in enumerate(test_devices):
            ...
            with self.session():
                ...
                self.evaluate(
                    var1.tables[0].save_to_file_system(
                        dirpath, file_name="lfsv_" + str(idx), buffer_size=4096
                    )
                )
                self.evaluate(
                    var2.tables[0].load_from_file_system(
    ...
    def test_cuckoo_hashtable_save_and_load_all_with_local_file_system(self):
        ...
        for idx, devices in enumerate(test_devices):
            ...
            with self.session():
                ...
                self.evaluate(
                    var1.tables[0].save_to_file_system(dirpath, buffer_size=1000)
                )
                self.evaluate(
                    var1.tables[1].save_to_file_system(dirpath, buffer_size=1000)
                )
                self.evaluate(
                    var2.tables[0].load_from_file_system(
                        dirpath,
